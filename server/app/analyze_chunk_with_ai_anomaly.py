...
def analyze_all_anomalies(chunk_data: dict) -> dict:
    ...
    prompt_text = f"""
你是一个多维度小组协作分析专家，专门分析**当前用户**在小组讨论中的异常状态，并对比其他组员，并对你自己的分析内容进行自我打分。
...
{{
  ...
  "score": {{
    ...
    "提示间隔惩罚": 30,
    "总评分": 67.25,
    "评分原因": "数据充分，异常判断准确；建议具体可操作；但存在一定重复和频繁提示",
    "是否提示": true
  }}
}}
...
💡 置信度：数据充分且证据清晰≥85，数据片面≤70，无历史支撑<50（权重0.70）
💡 建议明确性：具体行动步骤≥80，模糊建议≤60（权重0.25）
💡 输出重复惩罚：高相似度≥70，低相似度≤20（权重0.35）
💡 提示间隔惩罚：<4分钟≥30，≥4分钟=0（权重0.25）

总评分 = (置信度 * 0.70) + (建议明确性 * 0.25) - (输出重复惩罚 * 0.35) - (提示间隔惩罚 * 0.25)
