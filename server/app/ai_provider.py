...
def generate_response(bot_id: str, main_prompt: str, history_prompt: str = None, prompt_type: str = "cognitive_guidance", model: str = "grok-2-latest", api_provider: str = None):
    """
    统一管理 AI API 调用，支持 xAI、HKUST GZ 和 Gemini 之间的切换。

    - `bot_id`: 识别调用的机器人 ID
    - `prompt`: 需要 AI 处理的文本内容
    - `prompt_type`: 使用的 prompt 类型，例如 `cognitive_guidance`
    ...
    if api_provider == "xai":
        return generate_xai_response(bot_id=bot_id, main_prompt=main_prompt, history_prompt=history_prompt, prompt_type=prompt_type, model=model)
    elif api_provider == "hkust_gz":
        return generate_hkust_response(bot_id=bot_id, main_prompt=main_prompt, history_prompt=history_prompt, prompt_type=prompt_type, model=model)
    elif api_provider == "gemini":
        return generate_gemini_response(bot_id=bot_id, main_prompt=main_prompt, history_prompt=history_prompt, prompt_type=prompt_type, model=model)
    else:
